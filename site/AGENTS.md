...
`site/vercel.json` is **generated by `scripts/write-headers.mjs`, and committed.** Unlike
`_headers` / `_redirects` / `.htaccess`, which are written into `dist/`, Vercel reads its
config from the repository at deploy time — so this one has to be in git. Run the build and
commit the file if it changed; the build prints `** CHANGED — git add site/vercel.json **`
when it does. Never hand-edit it: the CSP inside depends on `FORM_ENDPOINT` and `GTM_ID`.

Vercel project settings that are not defaults: **Root Directory must be `site`** (package.json
is not at the repo root), and "Include files outside the Root Directory" must stay on, because
the build script calls `../scripts/write-headers.mjs`.
